...
1. [Cài đặt Maven](#1-cài-đặt-maven)
2. [Cài đặt GraalVM native-image](#2-cài-đặt-graalvm) (Thay thế Java SDK)
3. [Cài đặt các yêu cầu cho GraalVM native-image (Production)](#3-cài-đặt-các-yêu-cầu-cho-graalvm-native-image-để-xây-dựng-bản-production)
...
1.1 Nhấn vào cài đặt, sau đó nhấn "Next".
...
#### 3.2. Nếu lệnh trên trả về phiên bản psql thì tiếp tục phần tiếp theo

```pwsh
createdb -U postgres book-management
```

Nhập mật khẩu của bạn từ [#1.4](#1-cài-đặt-postgresql).
...
- Tải Maven từ <https://maven.apache.org/download.cgi>
  - Tệp zip nhị phân `apache-maven-3.9.9-bin.zip`
...
- Thêm đường dẫn Maven vào biến môi trường hệ thống
  - Mở biến môi trường
  - Nhấp vào `Path` trong phần người dùng
  - Nhấn "New"
